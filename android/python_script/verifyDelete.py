...
def verify_deletion():
    """Verify that the specified fields have been deleted from all documents."""
    all_verified = True
    ...
    for doc in docs:
        ...
        for field in fields_to_delete:
            if field in data:
                print(f"Field '{field}' still exists in document ID: {doc.id}")
                all_verified = False

    if all_verified:
        print("All fields successfully deleted from all documents.")
    else:
        print("Some fields were not deleted. Please check the logs.")

if __name__ == "__main__":
    delete_fields()
    print("\nVerifying field deletions...")
    verify_deletion()
