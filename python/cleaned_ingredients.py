...
def is_food_noun(word):
    ...
    """
    for synset in wn.synsets(word, pos=wn.NOUN):  # Check only noun synsets
...
def clean_ingredients(ingredients_list):
    ...
    for ingredient in ingredients_list:
        words = word_tokenize(ingredient)  # Tokenize the ingredient into words
        for word in words:
...
def safely_parse_ingredients(raw_ingredients):
    ...
    # Fallback: Check if the string looks like a list and split manually
...
def process_and_update_firestore():
    ...
    """
    recipes_ref = db.collection('recipes')  # Reference to the recipes collection
    ...
    while True:
        ...
        recipes = list(query.stream())  # Get the current batch
        ...
        for recipe in recipes:
            try:
                ...
                if 'ingredients' in data:  # Check if the ingredients field exists
                    ...
                    cleaned = clean_ingredients(ingredients_list)  # Clean the ingredients
